...
>
> **⚠️ 光补 `--mm-*` 还不够（Task 5 审查发现）**：生成层里还有**硬编码色值**，补令牌修不动它们，必须在自有层用令牌覆盖。已发现一处：**`.mm-stat-left`（底栏左侧「字数 / 节点数」）的底色是硬编码 `rgba(255,255,255,.9)`**。做法与其他"生成层皮 + 自有层内容"问题一致：自有层用 `var(--mm-surface-2)` 一类令牌覆盖，并注明「生成层是硬编码、此处按主题令牌重述」。**清扫范围要含两处，别只扫生成层**：
> - 生成层导图段（`components.css:652–1484`）：`grep -n "rgba(255\|rgba(0, 0, 0\|#[0-9a-f]\{6\}" src/styles/components.css` 后取该段；
> - **自有层 `mindmap.css`**（Task 5 审查补）：已知 `.mm-ctx-sub` 的 `box-shadow: … rgba(0,0,0,.5)`（`mindmap.css:438`）与 `.mm-ctx` 沿用的生成层 `rgba(0,0,0,.45)` 阴影偏重（浅色主题下过重、深色下不可见）。
>
> **措辞订正（Task 5 审查，不改结论）**：对**底栏**而言深色主题下不是「白底浅字」而是「白底 + 深字」（底栏所有文字取 `--mm-*` ⇒ 深色主题下仍是 `#333`）——**能读、但离题**（其它窗口都是深底）。补 30 套 `--mm-*` 的结论不变。
...
>    - 大纲全屏编辑 40 → 拖拽遮罩 50 → 各弹窗 2000/3000 → toast 10001
>    - **右键菜单已不在窗口层叠上下文里（Task 5 更新）**：它改挂 body 后继承生成层的 **9760**（`.mm-ctx`），**不再是我先前记的 60**；副作用是它与「弹窗 2000/3000」的相对次序**翻转**了（Task 5 审查追过两条可能的坏结果：会压过 9760 的 `#mindmapWindow.mm-fullscreen` 9800 是全仓无引用的死规则；图片预览 3000 那条被「单击冒泡成 `node_click` 先关菜单」挡住 ⇒ 目前不可达）。**裁决时以 9760 为准并写明这个翻转与它当前为何无害**，不要去把它「恢复成 60」（那会把它压回窗口内浮层之下）。
...
>
> 4. **底栏小地图按钮的既有隐患（Task 5 审查发现，非本阶段引入）**：`openMiniMap` 是底栏的**本地状态**，而 `Navigator.vue:60-68` 是**赋值**而非取反；禅模式进出会让底栏卸载重挂（`MindMapApp.vue:495`）⇒ `openMiniMap` 复位为 false 而小地图仍开着 ⇒ 出来后第一次点该按钮「没反应」（第二次才关）。两种改法：把小地图开合状态提到 `ui` store，或让按钮的 active 与点击都读 Navigator 的真实状态。**归本任务或 Task 8 实机确认后修**。
