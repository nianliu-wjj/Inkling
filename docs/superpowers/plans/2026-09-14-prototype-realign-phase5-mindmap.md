...
>
> **⚠️ 光补 `--mm-*` 还不够（Task 5 审查发现）**：生成层里还有**硬编码色值**，补令牌修不动它们，必须在自有层用令牌覆盖。已发现一处：**`.mm-stat-left`（底栏左侧「字数 / 节点数」）的底色是硬编码 `rgba(255,255,255,.9)`**，13 套深色主题下会白底浅字。做法与其他"生成层皮 + 自有层内容"问题一致：自有层 `:root` 作用域下用 `var(--mm-surface-2)` 一类令牌覆盖，并注明「生成层是硬编码、此处按主题令牌重述」。**实施时请顺带 grep 一遍生成层的 `.mm-*` 区块里其它硬编码色值**（`grep -n "rgba(255\|rgba(0, 0, 0\|#[0-9a-f]\{6\}" src/styles/components.css` 的导图段 652–1484），一并列出、同一节覆盖。
