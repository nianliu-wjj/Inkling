...
## 5.1 已知遗留（同类残留，未修，留待后续）

修复批在改「保存失败不再报成功」时，暴露了两处**同类**的「失败仍报成功」残留。它们不在整分支审查点名的范围内（审查只点了保留天数那一处），为遵守「不顺手重构」原则未一并收掉：

| 位置 | 现象 |
|---|---|
| `LauncherPageView.vue:84` 的 `persist` 包装 | 快捷键保存失败时，`patch` 先弹「保存设置失败」，`useShortcutRecorder` 随后仍 toast「启动器快捷键已设为 X」——双 toast 且后一条为假 |
| `useLauncherSettings.ts:83` 的 `saveLauncherRoots` | `patch` 失败后仍继续 `rebuildLauncher()`——用未生效的根目录配置重建索引 |

两者的失败代价都只是提示失真（数据不会被写坏），但**与本次修掉的保留天数问题属同一类**，建议在阶段五一并清理。
