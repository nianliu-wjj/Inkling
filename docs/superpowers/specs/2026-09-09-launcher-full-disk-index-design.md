...
**采用（严格分组，用户定稿）**：结果先按类别分桶并固定桶顺序 **应用组 → 文件夹 → 文件**；每桶**内部**按匹配分降序（同分再按历史加权）。

- 桶优先级：`App / Uwp / Command = 0`（可执行/直达类，置最前），`Folder = 1`，`File = 2`。命令属 Inkling 功能直达，与应用同属「可执行」，并入应用组。
- 最终比较器：先比桶优先级（小者在前），再比匹配分（高者在前），再比历史加权。
- 空查询回退：保留现有「只出应用/UWP」逻辑。
- 每桶各自截断上限，避免文件把应用挤出可视区。

**理由**：用户明确要求严格分组顺序（应用 > 文件夹 > 文件），焦点落点可预测、形成肌肉记忆；「回车打开第一项」自然落在最相关的应用上。

**放弃的备选**：匹配分为主序、仅同分档内按类别倾斜（会让完美匹配的文件排到应用前，与用户要的严格顺序不符）。
