...
/// 导图窗口默认逻辑尺寸。
///
/// 三段式顶栏（左岛 + 文件名岛 + 右岛）按生成层的实际字号实测下界约 1332px
/// （左岛 17 枚 893px + 右岛 5 枚 256px + 中岛 120–320px + `.mm-topbar` 的 gap/内距 56px），
/// 故默认宽取 1360（≈ 下界 + 28px）。中岛（文件名）越长越挤：超过约 148px 后左岛开始缩，
/// 到中岛上限 320（`MAP_NAME_LIMIT` 32 字）时合计约 1532 > 1360，左岛会出内部横向滚动条
/// —— 这是刻意的降级顺序（宁可左岛滚动，也不让右岛的保存/导出/关闭被裁，见 `mindmap.css`
/// 的顶栏拓扑）。原型的 `min(97vw, 1180px)` 在自家宽度下同样会让两岛内部滚动。
const MINDMAP_SIZE: (f64, f64) = (1360.0, 900.0);
